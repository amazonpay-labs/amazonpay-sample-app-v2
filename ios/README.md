> [!NOTE]
[2023/11/10 追記] 2023/9/29以降のバージョンでAndroid側に実装されたアプリ切替時の挙動の改善のうち、一部iOS側でも対処が必要なことが判明いたしました。  

[中継用ページのアプリに自動で戻る処理が起動しなかった場合に備えて、戻る用のボタンを表示する対処](../android/README_fixSwitchApp.md#3-中継用ページのamazonpayactivityの自動起動が発動しなかった場合の対処)がそちらで、iOSでも稀に同問題が発生するため同対処を実装する必要があります。  
